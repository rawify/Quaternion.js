...
b=2*Math.acos(a);a=Math.sqrt(1-a*a);if(1E-16>a)return[[this.x,this.y,this.z],0];a=1/a;return[[this.x*a,this.y*a,this.z*a],b]},toEuler:function(a){function b(v){return 1<=v?Math.PI/2:-1>=v?-Math.PI/2:Math.asin(v)}var c=this.w,d=this.x,e=this.y,f=this.z,g=c*d,h=c*e;c*=f;var l=d*d,n=d*e;d*=f;var r=e*e;e*=f;f*=f;return void 0===a||"ZXY"===a?[-Math.atan2(2*(n-c),1-2*(l+f)),b(2*(e+g)),-Math.atan2(2*(d-h),1-2*(l+r))]:"XYZ"===a?[-Math.atan2(2*(e-g),1-2*(l+r)),b(2*(d+h)),-Math.atan2(2*(n-c),1-2*(r+f))]:"YXZ"===
...
e*g),l=2*(e*f-c*a),n=2*(c*g-d*f);return[f+b*h+d*n-e*l,g+b*l+e*h-c*n,a+b*n+c*l-d*h]},slerp:function(a,b,c,d){t(k,a,b,c,d);var e=this.w,f=this.x,g=this.y,h=this.z,l=k.w,n=k.x,r=k.y,v=k.z,x=e*l+f*n+g*r+h*v;0>x&&(e=-e,f=-f,g=-g,h=-h,x=-x);if(x>=1-1E-16)return function(q){return u(e+q*(l-e),f+q*(n-f),g+q*(r-g),h+q*(v-h))};var A=Math.acos(x),B=Math.sin(A);return function(q){var w=A*q;q=Math.sin(w);w=Math.cos(w)-x*q/B;q/=B;return m(w*e+q*l,w*f+q*n,w*g+q*r,w*h+q*v)}}};p.ZERO=m(0,0,0,0);p.ONE=m(1,0,0,0);p.I=
m(0,1,0,0);p.J=m(0,0,1,0);p.K=m(0,0,0,1);p.fromAxisAngle=function(a,b){var c=a[0],d=a[1],e=a[2],f=.5*b,g=Math.sin(f)/Math.sqrt(c*c+d*d+e*e);return m(Math.cos(f),c*g,d*g,e*g)};p.fromBetweenVectors=function(a,b){var c=a[0],d=a[1],e=a[2],f=b[0],g=b[1],h=b[2],l=Math.sqrt(c*c+d*d+e*e),n=Math.sqrt(f*f+g*g+h*h);0<l&&(c/=l,d/=l,e/=l);0<n&&(f/=n,g/=n,h/=n);l=c*f+d*g+e*h;return l>=1-1E-16?p.ONE:1E-16>=1+l?Math.abs(c)>Math.abs(e)?u(0,-d,c,0):u(0,0,-e,d):u(1+l,d*h-e*g,e*f-c*h,c*g-d*f)};p.random=function(){var a=
...
e*b*a-f*g*c,g*c*b-e*f*a):"ZYX"===d||"RPY"===d?m(e*f*g+c*b*a,g*c*b-e*f*a,e*g*b+f*c*a,e*b*a-f*g*c):"YZX"===d?m(c*b*a-e*f*g,e*f*a+g*c*b,e*b*a+f*g*c,f*c*a-e*g*b):"XZY"===d?m(e*f*g+c*b*a,e*b*a-f*g*c,g*c*b-e*f*a,e*g*b+f*c*a):null};p.fromMatrix=function(a){if(9===a.length){var b=a[0],c=a[1],d=a[2],e=a[3],f=a[4],g=a[5],h=a[6],l=a[7];a=a[8]}else b=a[0][0],c=a[0][1],d=a[0][2],e=a[1][0],f=a[1][1],g=a[1][2],h=a[2][0],l=a[2][1],a=a[2][2];var n=b+f+a;return 0<n?u(n+1,l-g,d-h,e-c):b>f&&b>a?u(l-g,1+b-f-a,c+e,d+h):
f>a?u(d-h,c+e,1+f-b-a,g+l):u(e-c,d+h,g+l,1+a-b-f)};"function"===typeof define&&define.amd?define([],function(){return p}):"object"===typeof exports?(Object.defineProperty(p,"__esModule",{value:!0}),p["default"]=p,p.Quaternion=p,module.exports=p):C.Quaternion=p})(this);