/*
Quaternion.js v1.6.5 10/9/2024
...
2*(h+d),2*(e-f),2*(c-g),2*(e+f),1-2*(h+p)]},toMatrix4:function(a){var b=this.w,c=this.x,e=this.y,d=this.z;const f=b*c,g=b*e;b*=d;const h=c*c,l=c*e;c*=d;const p=e*e;e*=d;d*=d;return a?[[1-2*(p+d),2*(l-b),2*(c+g),0],[2*(l+b),1-2*(h+d),2*(e-f),0],[2*(c-g),2*(e+f),1-2*(h+p),0],[0,0,0,1]]:[1-2*(p+d),2*(l-b),2*(c+g),0,2*(l+b),1-2*(h+d),2*(e-f),0,2*(c-g),2*(e+f),1-2*(h+p),0,0,0,0,1]},toCSSTransform:function(){var a=this.w;let b=2*Math.acos(a);a=1-a*a;1E-16>a?(b=0,a=1):a=1/Math.sqrt(a);return"rotate3d("+
this.x*a+","+this.y*a+","+this.z*a+","+b+"rad)"},toAxisAngle:function(){const a=this.w;var b=1-a*a;if(1E-16>b)return[[this.x,this.y,this.z],0];b=1/Math.sqrt(b);return[[this.x*b,this.y*b,this.z*b],2*Math.acos(a)]},toEuler:function(a){function b(v){return 1<=v?Math.PI/2:-1>=v?-Math.PI/2:Math.asin(v)}var c=this.w,e=this.x,d=this.y,f=this.z;const g=c*e,h=c*d;c*=f;const l=e*e,p=e*d;e*=f;const r=d*d;d*=f;f*=f;return void 0===a||"ZXY"===a?[-Math.atan2(2*(p-c),1-2*(l+f)),b(2*(d+g)),-Math.atan2(2*(e-h),1-
2*(l+r))]:"XYZ"===a||"RPY"===a?[-Math.atan2(2*(d-g),1-2*(l+r)),b(2*(e+h)),-Math.atan2(2*(p-c),1-2*(r+f))]:"YXZ"===a?[Math.atan2(2*(e+h),1-2*(l+r)),-b(2*(d-g)),Math.atan2(2*(p+c),1-2*(l+f))]:"ZYX"===a||"YPR"===a?[Math.atan2(2*(p+c),1-2*(r+f)),-b(2*(e-h)),Math.atan2(2*(d+g),1-2*(l+r))]:"YZX"===a?[-Math.atan2(2*(e-h),1-2*(r+f)),b(2*(p+c)),-Math.atan2(2*(d-g),1-2*(l+f))]:"XZY"===a?[Math.atan2(2*(d+g),1-2*(l+f)),-b(2*(p-c)),Math.atan2(2*(e+h),1-2*(r+f))]:null},clone:function(){return m(this.w,this.x,this.y,
this.z)},rotateVector:function(a){const b=this.w,c=this.x,e=this.y,d=this.z,f=a[0],g=a[1];a=a[2];let h=e*a-d*g,l=d*f-c*a,p=c*g-e*f;h+=h;l+=l;p+=p;return[f+b*h+e*p-d*l,g+b*l+d*h-c*p,a+b*p+c*l-e*h]},slerp:function(a,b,c,e){t(k,a,b,c,e);let d=this.w,f=this.x,g=this.y,h=this.z,l=k.w,p=k.x,r=k.y,v=k.z,x=d*l+f*p+g*r+h*v;0>x&&(d=-d,f=-f,g=-g,h=-h,x=-x);if(x>=1-1E-16)return function(q){return u(d+q*(l-d),f+q*(p-f),g+q*(r-g),h+q*(v-h))};let A=Math.acos(x),B=Math.sin(A);return function(q){var w=A*q;q=Math.sin(w);
w=Math.cos(w)-x*q/B;q/=B;return m(w*d+q*l,w*f+q*p,w*g+q*r,w*h+q*v)}}};n.ZERO=m(0,0,0,0);n.ONE=m(1,0,0,0);n.I=m(0,1,0,0);n.J=m(0,0,1,0);n.K=m(0,0,0,1);n.fromAxisAngle=function(a,b){const c=a[0],e=a[1];a=a[2];b*=.5;const d=Math.sin(b)/Math.sqrt(c*c+e*e+a*a);return m(Math.cos(b),c*d,e*d,a*d)};n.fromBetweenVectors=function(a,b){let c=a[0],e=a[1];a=a[2];let d=b[0],f=b[1];b=b[2];var g=Math.sqrt(c*c+e*e+a*a);const h=Math.sqrt(d*d+f*f+b*b);0<g&&(c/=g,e/=g,a/=g);0<h&&(d/=h,f/=h,b/=h);g=c*d+e*f+a*b;return g>=
1-1E-16?n.ONE:1E-16>=1+g?Math.abs(c)>Math.abs(a)?u(0,-e,c,0):u(0,0,-a,e):u(1+g,e*b-a*f,a*d-c*b,c*f-e*d)};n.random=function(){var a=Math.random();const b=Math.random(),c=Math.random(),e=Math.sqrt(1-a);a=Math.sqrt(a);return m(a*Math.cos(2*Math.PI*c),e*Math.sin(2*Math.PI*b),e*Math.cos(2*Math.PI*b),a*Math.sin(2*Math.PI*c))};n.fromEulerLogical=function(a,b,c,e){return n.fromEuler(c,b,a,void 0!==e?e[2]+e[1]+e[0]:e)};n.fromEuler=function(a,b,c,e){var d=.5*a,f=.5*b,g=.5*c;c=Math.cos(d);b=Math.cos(f);a=Math.cos(g);
